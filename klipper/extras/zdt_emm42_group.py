# zdt_emm42_group.py
# Coordinate three zdt_emm42 instances used by an independently driven Z axis.
...
class ZdtEmm42Group:
    def __init__(self, config):
        ...
        if len(self.member_names) != 3 or len(set(self.member_names)) != 3:
            raise config.error(
                'zdt_emm42_group: members must name exactly three unique '
                'zdt_emm42 instances')
        ...
        self.gcode.register_mux_command(
            'ZDT_EMM_GROUP_STATUS', 'NAME', self.name, self.cmd_STATUS,
            desc='Report synchronized state for a three-motor ZDT EMM42 group')
        self.gcode.register_mux_command(
            'ZDT_EMM_GROUP_AUTOTUNE', 'NAME', self.name,
            self.cmd_AUTOTUNE,
            desc='Coordinate position-loop PID tuning for a three-motor Z axis')
        self.gcode.register_mux_command(
            'ZDT_EMM_GROUP_AUTOTUNE_CANCEL', 'NAME', self.name,
            self.cmd_AUTOTUNE_CANCEL,
            desc='Request cancellation of active three-motor Z PID tuning')
        ...
        for name in self.member_names:
            ...
            try:
                ...
                raise self.printer.config_error(
                    "zdt_emm42_group %s: member '%s' was not found" %
                    (self.name, object_name))
            self.members.append(member)
        addresses = [member.addr for member in self.members]
        if len(set(addresses)) != len(addresses):
            raise self.printer.config_error(
                'zdt_emm42_group %s: member CAN addresses must be unique' %
                self.name)
    ...
    def _sample_timer(self, eventtime):
        if not self.monitor_suspended:
            try:
                ...
                logging.exception(
                    'zdt_emm42_group %s: sample aggregation failed',
                    self.name)
    ...
    def _member_snapshot(self, member, status):
        return {
            'name': member.name,
            'object': self._member_object_name(member.name),
            'addr': member.addr,
        ...
        if reasons:
            ...
            if self.warning_active:
                ...
                if (changed or eventtime - self.last_warning_log_time >=
                    ...
                    logging.warning(
                        'zdt_emm42_group %s warning: %s', self.name,
                        self.warning_reasons)
    ...
    def _latest_aligned_samples(self):
        ...
        for member in self.members:
            if not member.error_history:
                return None
            samples.append(member.error_history[-1])
        times = [float(sample['time']) for sample in samples]
        if max(times) - min(times) > self.sample_skew_tolerance:
            return None
        if any(self.last_sample_times.get(member.name) == sample['time']
    ...
    def _collect_sample(self, eventtime):
        if len(self.members) != getattr(self, 'expected_member_count', 3):
            return
        statuses = [member.get_status(eventtime) for member in self.members]
        snapshots = [
        ...
        if aligned is not None and self.last['online']:
            ...
            aligned_mm = [float(value['error_mm']) for value in aligned]
            sample = {
                ...
                'max_abs_error_mm': max(abs(value)
                                        for value in aligned_mm),
                'spread_deg': max(aligned_deg) - min(aligned_deg),
                'spread_mm': max(aligned_mm) - min(aligned_mm),
            }
            self.history.append(sample)
            for member, value in zip(self.members, aligned):
                self.last_sample_times[member.name] = value['time']
    ...
    def cmd_STATUS(self, gcmd):
        ...
        lines = [
            "ZDT EMM42 group '%s' online=%s warning=%s" % (
    ...
    def _check_autotune_preconditions(self, gcmd, distance):
        if len(self.members) != 3:
            raise gcmd.error('ZDT EMM42 group is not connected')
        ...
        originals = []
        for member in self.members:
            if member.autotune_active:
                raise gcmd.error(
                    "zdt_emm42 '%s' is already tuning" % member.name)
            if member.can_payload_includes_addr:
                raise gcmd.error(
                    "zdt_emm42 '%s' must use "
                    'can_payload_includes_addr=False' % member.name)
            data = member._query_sync(
                0x37, timeout=member.query_timeout)
            if data is None or not member._record_valid_response(
                    0x37, data, self.reactor.monotonic()):
                raise gcmd.error(
                    "zdt_emm42 '%s' CAN preflight failed: %s" %
                    (member.name, member.last_error))
            status = member.get_status(self.reactor.monotonic())
            if not status.get('online'):
                raise gcmd.error(
                    "zdt_emm42 '%s' is offline" % member.name)
            if status.get('stalled') or status.get('stall_protect'):
                raise gcmd.error(
                    "zdt_emm42 '%s' reports a stall" % member.name)
            pid = member._read_pid()
            if pid is None or not member._write_pid(
                    pid, store=0, verify=True):
                raise gcmd.error(
                    "zdt_emm42 '%s' PID read/write preflight failed: %s" %
                    (member.name, member.last_error))
    ...
    def _prepare_member_capture(self):
        ...
        for member in self.members:
            saved.append({
                ...
                'error_poll_interval': member.error_poll_interval,
                'history_maxlen': member.error_history.maxlen,
            })
            member.autotune_active = True
            member.enabled = True
            member.pending_cmd = None
            member.pending_error_cmd = None
            member._cancel_pid_request()
            member.error_poll_interval = AUTOTUNE_SAMPLE_INTERVAL
            member._resize_error_history(max(
                2, int(math.ceil(GROUP_HISTORY_SECONDS /
                                 AUTOTUNE_SAMPLE_INTERVAL)) + 2))
            if member.timer is not None:
                member.reactor.update_timer(
                    member.timer, member.reactor.NEVER)
            if member.error_timer is not None:
                member.reactor.update_timer(
                    member.error_timer, member.reactor.NEVER)
            if member.pid_timer is not None:
                member.reactor.update_timer(
                    member.pid_timer, member.reactor.NEVER)
        return saved

    def _restore_member_capture(self, saved):
        for member, state in zip(self.members, saved):
            if member.autotune_capture_active:
                member._end_autotune_capture()
            member.error_poll_interval = state['error_poll_interval']
            member._prune_error_history(self.reactor.monotonic())
            member._resize_error_history(state['history_maxlen'])
            member.autotune_active = False
            member.autotune_abort = False
            member.enabled = state['enabled']
            if state['enabled']:
                member.reactor.update_timer(
                    member.timer, member.reactor.NOW)
                member.reactor.update_timer(
                    member.error_timer, member.reactor.NOW)
                member.reactor.update_timer(
                    member.pid_timer, member.reactor.monotonic() + 0.25)
            else:
                member._set_polling_enabled(False)
        self.monitor_suspended = False
    ...
    def _align_capture_samples(self, captures):
        if not captures or any(not values for values in captures):
            return []
        aligned = []
    ...
    def _run_motion(self, toolhead, origin, distance, speed, accel,
        ...
        returned = False
        for member in self.members:
            member._begin_autotune_capture(self.max_error_deg)
            member.autotune_capture_phase = 'motion:z'
        try:
            ...
            returned = True
            for member in self.members:
                member.autotune_capture_phase = 'settle'
            self.reactor.pause(
            ...
            if not returned:
                try:
                    ...
                    logging.exception(
                        'zdt_emm42_group %s: failed to return Z after tuning',
                        self.name)
            captures = []
            for member in self.members:
                captures.append([
                    dict(sample) for sample in
                    member.autotune_capture_samples])
                member._end_autotune_capture()
        ...
        if self.autotune_abort:
            raise RuntimeError('3Z autotune cancellation requested')
        for member in self.members:
            violation = member.autotune_safety_violation
            member.autotune_safety_violation = None
            if violation:
                raise GroupCandidateRejected(
                    "%s: %s" % (member.name, violation))
            status = member.get_status(self.reactor.monotonic())
            if not status.get('online'):
                raise RuntimeError(
                    "zdt_emm42 '%s' went offline during tuning" %
                    member.name)
            flags = member._query_sync(
                0x3A, timeout=member.query_timeout)
            if flags is None or not member._record_valid_response(
                    0x3A, flags, self.reactor.monotonic()):
                raise RuntimeError(
                    "failed to read motor flags for '%s'" % member.name)
            if member.last.get('stalled') or member.last.get('stall_protect'):
                raise RuntimeError(
                    "zdt_emm42 '%s' reported a stall" % member.name)
    ...
    def _write_pid_set(self, values, store=0, verify=True):
        for member, pid in zip(self.members, values):
            if not member._write_pid(pid, store=store, verify=verify):
                return False, member
        return True, None
    ...
    def _restore_pid_set(self, originals, store):
        ...
        for member, pid in zip(self.members, originals):
            try:
                if not member._write_pid(
                        pid, store=store, verify=False):
                    failures.append(member.name)
            except Exception:
                failures.append(member.name)
                logging.exception(
                    "zdt_emm42_group %s: failed to restore PID for '%s'",
    ...
    def _candidate(self, current, member_index, parameter, direction, step):
        ...
        if (value < member.autotune_pid_min or
                value > member.autotune_pid_max):
            direction *= -1
            value = old_value + direction * step
        value = max(member.autotune_pid_min,
                    min(member.autotune_pid_max, int(value)))
        if value == old_value:
    ...
    def _format_pid_set(self, values):
        return '; '.join(
            '%s=%s/%s/%s' % (
                member.name, pid[0], pid[1], pid[2])
            for member, pid in zip(self.members, values))

    def cmd_AUTOTUNE(self, gcmd):
        if self.autotune_active:
            raise gcmd.error('ZDT_EMM_GROUP_AUTOTUNE is already running')
        ...
        try:
            ...
            steps = [[
                member.autotune_kp_step,
                member.autotune_ki_step,
                member.autotune_kd_step,
            ] for member in self.members]
            ...
            if not ok:
                ...
                message = "failed to persist PID for '%s'" % failed_member.name
                if failures:
                    message += '; original restore failed for ' + ', '.join(
                        failures)
                raise gcmd.error(message)
            readback = [member._read_pid() for member in self.members]
            if readback != current:
                failures = self._restore_pid_set(originals, store=1)
                ...
                raise gcmd.error(message)
            gcmd.respond_info(
                'ZDT EMM42 3Z autotune complete: %s; training %s; '
            ...
            if originals is not None:
                self._restore_pid_set(
                    originals, store=1 if persist_attempted else 0)
            logging.exception(
                'zdt_emm42_group %s: unexpected autotune failure',
                self.name)
    ...
    def cmd_AUTOTUNE_CANCEL(self, gcmd):
        if not self.autotune_active:
            gcmd.respond_info('ZDT EMM42 3Z autotune is not running')
            return
        self.autotune_abort = True
        for member in self.members:
            member.autotune_abort = True
        gcmd.respond_info(
            'ZDT EMM42 3Z autotune cancellation requested; the current '
