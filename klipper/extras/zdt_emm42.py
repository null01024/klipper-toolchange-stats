...
WRITABLE_CONFIG_PARAMS = {
    ...
    'PULSE_MODE': ('pulse_mode', 0, 3),
    'EN_LEVEL': ('en_level', 0, 2),
    'DIR': ('dir', 0, 1),
    'MICROSTEPS': ('microsteps', 1, 256),
    'INTERPOLATION': ('interpolation', 0, 1),
    'OPEN_LOOP_CURRENT': ('open_loop_current_ma', 0, 3000),
    'STALL_MAX_CURRENT': ('stall_max_current_ma', 0, 3000),
    'MAX_OUTPUT_VOLTAGE': ('max_output_mv', 0, 5000),
    'STALL_PROTECTION': ('stall_protection', 0, 1),
    'STALL_RPM': ('stall_rpm', 0, 5000),
    'STALL_CURRENT': ('stall_current_ma', 0, 3000),
    'STALL_TIME': ('stall_time_ms', 0, 65535),
...
class ZdtEmm42:
    ...
    def _empty_status(self):
        return {
            ...
            'pid_error': '',
            'driver_config': {},
            'config_last_update_time': 0.0,
            'config_error': '',
            'config_write_status': None,
            'config_temporary': False,
            'settings_busy': False,
            'motor_flags': None,
            'enabled': None,
            'reached': None,
    ...
    def _read_driver_config(self, timeout=None):
        timeout = max(self.query_timeout, 0.05) if timeout is None else timeout
        data = self._query_long_sync(
            CMD_READ_CONFIG, bytes([CONFIG_READ_SUBCOMMAND]),
            response_len=CONFIG_RESPONSE_LEN, timeout=timeout)
        if data is None or not self._record_valid_response(
                CMD_READ_CONFIG, data, self.reactor.monotonic()):
            self.last['config_error'] = self.last_error or 'configuration read failed'
            return None
        return dict(self.last['driver_config'])

    def _write_driver_config(self, raw, store=0, verify=True, timeout=None):
        if store not in (0, 1):
            raise ValueError('driver configuration store flag must be 0 or 1')
        raw = bytearray(raw)
        if len(raw) != CONFIG_DATA_LEN:
            raise ValueError('driver configuration must contain 28 bytes')
        timeout = max(self.query_timeout, 0.05) if timeout is None else timeout
        extra = bytes([CONFIG_WRITE_SUBCOMMAND, store]) + bytes(raw)
        data = self._query_sync(
            CMD_WRITE_CONFIG, extra, timeout=timeout, send_long=True)
        if data is None or not self._record_valid_response(
                CMD_WRITE_CONFIG, data, self.reactor.monotonic()):
            self.last['config_error'] = self.last_error or 'configuration write failed'
            return False
        if not verify:
            return True
        readback = self._read_driver_config(timeout)
        if readback is None or self.driver_config_raw != raw:
            self.last_error = 'driver configuration readback mismatch'
            self.last['config_error'] = self.last_error
            return False
        return True
    ...
    def _check_settings_write_preconditions(self, gcmd):
        if gcmd.get_int('CONFIRM', 0, minval=0, maxval=1) != 1:
            raise gcmd.error('EMM42 settings write requires CONFIRM=1')
        if self.settings_busy or self.autotune_active:
            raise gcmd.error('EMM42 settings are busy or PID calibration is active')
        ...
        if print_stats is not None:
            ...
            if state not in ('standby', 'complete', 'cancelled', 'error'):
                raise gcmd.error(
                    'EMM42 settings require an idle printer (state=%s)' % state)
        error_data = self._query_sync(CMD_POS_ERROR, timeout=self.query_timeout)
        if error_data is None or not self._record_valid_response(
                CMD_POS_ERROR, error_data, self.reactor.monotonic()):
            raise gcmd.error('EMM42 settings CAN preflight failed: %s' % self.last_error)
        motor_data = self._query_sync(CMD_MOTOR_FLAGS, timeout=self.query_timeout)
        if motor_data is None or not self._record_valid_response(
                CMD_MOTOR_FLAGS, motor_data, self.reactor.monotonic()):
            raise gcmd.error('EMM42 motor-state preflight failed: %s' % self.last_error)
        if self.last.get('stalled') or self.last.get('stall_protect'):
            raise gcmd.error('EMM42 reports a stall or active stall protection')
        rpm_data = self._query_sync(CMD_RPM, timeout=self.query_timeout)
        if rpm_data is None or not self._record_valid_response(
                CMD_RPM, rpm_data, self.reactor.monotonic()):
            raise gcmd.error('EMM42 speed preflight failed: %s' % self.last_error)
        if int(self.last.get('rpm') or 0) != 0:
            raise gcmd.error('EMM42 settings require a stopped motor (rpm=%s)' %
                             self.last.get('rpm'))

    def _begin_settings_write(self, gcmd):
        self._check_settings_write_preconditions(gcmd)
        self.settings_busy = True
        self.last['settings_busy'] = True

    def _end_settings_write(self):
        self.settings_busy = False
        self.last['settings_busy'] = False

    def cmd_CONFIG_REFRESH(self, gcmd):
        if self.settings_busy or self.autotune_active:
            raise gcmd.error('EMM42 settings are busy or PID calibration is active')
        config = self._read_driver_config()
        pid = self._read_pid()
        if config is None or pid is None:
            raise gcmd.error('EMM42 settings refresh failed: %s' % self.last_error)
        gcmd.respond_info(
            "ZDT Emm42 '%s' settings refreshed; PID %s" %
            (self.name, self._format_pid(pid)))

    def cmd_CONFIG_SET(self, gcmd):
        self._begin_settings_write(gcmd)
        try:
            current = self._read_driver_config()
            if current is None or self.driver_config_raw is None:
                raise gcmd.error('failed to read current driver configuration: %s' %
                                 self.last_error)
            updates = {}
            for parameter, (field, minimum, maximum) in WRITABLE_CONFIG_PARAMS.items():
                raw_value = gcmd.get(parameter, None)
                if raw_value is None:
                    continue
                value = gcmd.get_int(parameter, minval=minimum, maxval=maximum)
                if field == 'motor_type' and value not in (0x19, 0x32):
                    raise gcmd.error('MOTOR_TYPE must be 25 (1.8 degree) or 50 (0.9 degree)')
                updates[field] = value
            if not updates:
                raise gcmd.error('no writable EMM42 configuration fields were supplied')
            store = gcmd.get_int('STORE', 0, minval=0, maxval=1)
            expected = _patch_driver_config(self.driver_config_raw, updates)
            if not self._write_driver_config(expected, store=store, verify=True):
                raise gcmd.error('EMM42 configuration write failed: %s' % self.last_error)
            self.last['config_temporary'] = store == 0
            gcmd.respond_info(
                "ZDT Emm42 '%s' configuration %s and verified" %
                (self.name, 'temporarily applied' if store == 0 else 'stored'))
        finally:
            self._end_settings_write()

    def cmd_PID_SET(self, gcmd):
        self._begin_settings_write(gcmd)
        try:
            pid = (
                gcmd.get_int('KP', minval=PID_MIN_VALUE, maxval=PID_MAX_VALUE),
                gcmd.get_int('KI', minval=PID_MIN_VALUE, maxval=PID_MAX_VALUE),
                gcmd.get_int('KD', minval=PID_MIN_VALUE, maxval=PID_MAX_VALUE),
            )
            store = gcmd.get_int('STORE', 0, minval=0, maxval=1)
            if not self._write_pid(pid, store=store, verify=True):
                raise gcmd.error('EMM42 PID write failed: %s' % self.last_error)
            self.last['config_temporary'] = store == 0
            gcmd.respond_info(
                "ZDT Emm42 '%s' PID %s and verified: %s" %
                (self.name, 'temporarily applied' if store == 0 else 'stored',
                 self._format_pid(pid)))
        finally:
            self._end_settings_write()

    def cmd_CONFIG_STORE(self, gcmd):
        self._begin_settings_write(gcmd)
        try:
            config = self._read_driver_config()
            pid = self._read_pid()
            if config is None or self.driver_config_raw is None or pid is None:
                raise gcmd.error('failed to read current EMM42 settings: %s' %
                                 self.last_error)
            if not self._write_driver_config(
                    self.driver_config_raw, store=1, verify=True):
                raise gcmd.error('failed to store EMM42 configuration: %s' %
                                 self.last_error)
            if not self._write_pid(pid, store=1, verify=True):
                raise gcmd.error(
                    'driver configuration was stored, but PID storage failed: %s' %
                    self.last_error)
            self.last['config_temporary'] = False
            gcmd.respond_info(
                "ZDT Emm42 '%s' current configuration and PID stored" % self.name)
        finally:
            self._end_settings_write()
