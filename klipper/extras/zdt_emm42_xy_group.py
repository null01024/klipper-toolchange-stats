...
# Coordinate two ZDT EMM42 motors used by a standard CoreXY drive.
...
try:
    from . import zdt_emm42_group as group_base
except ImportError:
    import zdt_emm42_group as group_base
...
class ZdtEmm42XYGroup(group_base.ZdtEmm42Group):
    def __init__(self, config):
        ...
        if len(self.member_names) != 2 or len(set(self.member_names)) != 2:
            raise config.error(
                'zdt_emm42_xy_group: members must name exactly two unique '
                'zdt_emm42 instances')
        ...
        self.gcode.register_mux_command(
            'ZDT_EMM_XY_GROUP_STATUS', 'NAME', self.name, self.cmd_STATUS,
            desc='Report synchronized state for a two-motor CoreXY EMM42 group')
        self.gcode.register_mux_command(
            'ZDT_EMM_XY_GROUP_AUTOTUNE', 'NAME', self.name,
            self.cmd_AUTOTUNE,
            desc='Coordinate position-loop PID tuning for two CoreXY motors')
        self.gcode.register_mux_command(
            'ZDT_EMM_XY_GROUP_AUTOTUNE_CANCEL', 'NAME', self.name,
            self.cmd_AUTOTUNE_CANCEL,
            desc='Request cancellation of active CoreXY PID tuning')

    def _empty_status(self):
        status = super()._empty_status()
        status['group_type'] = 'corexy'
        return status
    ...
    def _handle_connect(self):
        try:
            ...
            logging.exception(
                'zdt_emm42_xy_group %s: failed to resolve members', self.name)
            raise
    ...
    def _sample_timer(self, eventtime):
        if not self.monitor_suspended:
            try:
                ...
                logging.exception(
                    'zdt_emm42_xy_group %s: sample aggregation failed',
                    self.name)
    ...
    def cmd_STATUS(self, gcmd):
        ...
        lines = [
            "ZDT EMM42 CoreXY group '%s' online=%s warning=%s" % (
    ...
    def _check_autotune_preconditions(self, gcmd, distance):
        if len(self.members) != self.expected_member_count:
            raise gcmd.error('ZDT EMM42 CoreXY group is not connected')
        ...
        for member in self.members:
            ...
            data = member._query_sync(0x37, timeout=member.query_timeout)
    ...
    def _run_profile(self, toolhead, origin, distance, speed, accel,
        ...
        for member in self.members:
            ...
            member.autotune_capture_phase = 'motion:' + profile
        try:
            saved_limits = self._set_motion_limits(toolhead, speed, accel)
            route = self.members[0]._corexy_route(
            ...
            if not returned:
                try:
                    ...
                    logging.exception(
                        'zdt_emm42_xy_group %s: failed to return after profile',
                        self.name)
            for member in self.members:
                captures.append([
        ...
        for member in self.members:
            ...
            if violation:
                violations.append('%s: %s' % (member.name, violation))
        if violations:
        ...
        for member in self.members:
            ...
            if not status.get('online'):
                raise RuntimeError(
                    ...
                    member.name)
            flags = member._query_sync(0x3A, timeout=member.query_timeout)
    ...
    def _pid_bounds(self, gcmd, originals, steps):
        ...
        for member_index, (member, original) in enumerate(zip(
            ...
            defaults = [
                (max(member.autotune_pid_min, int(original[0] * 0.5)),
                 min(member.autotune_pid_max,
                     max(1, int(original[0] * 2.0)))),
                (member.autotune_pid_min,
                 min(member.autotune_pid_max,
                     max(int(original[1] * 10),
                         original[1] + 10 * steps[member_index][1]))),
                (max(member.autotune_pid_min, int(original[2] * 0.5)),
                 min(member.autotune_pid_max,
    ...
    def cmd_AUTOTUNE(self, gcmd):
        if self.autotune_active:
            raise gcmd.error('ZDT_EMM_XY_GROUP_AUTOTUNE is already running')
        ...
        try:
            ...
            if readback != current:
                ...
                raise gcmd.error(message)
            gcmd.respond_info(
                'ZDT EMM42 CoreXY autotune complete: %s; training %s; '
            ...
            if originals is not None:
                self._restore_pid_set(
                    originals, store=1 if persist_attempted else 0)
            logging.exception(
                'zdt_emm42_xy_group %s: unexpected autotune failure',
                self.name)
            ...
            if moved_to_origin and toolhead is not None and start is not None:
                try:
                    ...
                    logging.exception(
                        'zdt_emm42_xy_group %s: failed to return to start',
                        self.name)
    ...
    def cmd_AUTOTUNE_CANCEL(self, gcmd):
        if not self.autotune_active:
            gcmd.respond_info('ZDT EMM42 CoreXY autotune is not running')
            return
