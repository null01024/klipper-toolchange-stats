...
def make_monitor():
    ...
    monitor.driver_config_raw = None
    monitor.settings_busy = False
...
class ZdtEmm42Test(unittest.TestCase):
    def test_driver_configuration_response_is_parsed(self):
        ...
        self.assertEqual(values['pulse_mode'], 0x02)
        self.assertEqual(values['serial_mode'], 0x03)
        self.assertEqual(values['microsteps'], 16)
        self.assertEqual(values['open_loop_current_ma'], 1000)
        self.assertEqual(values['stall_max_current_ma'], 3000)
        self.assertEqual(values['max_output_mv'], 4000)
        self.assertEqual(values['can_baud_index'], 7)
        self.assertEqual(values['device_address'], 1)
        self.assertEqual(values['stall_current_ma'], 2400)
        self.assertEqual(values['stall_time_ms'], 4000)
        self.assertEqual(values['position_window_tenths'], 1)

    def test_driver_configuration_patch_preserves_read_only_fields(self):
        ...
        updated = ZDT._patch_driver_config(original, {
            ...
            'stall_protection': 0,
            'position_window_tenths': 3,
        })

        self.assertEqual(updated[5], 0)
        self.assertEqual(updated[8:10], bytes.fromhex('04 B0'))
        self.assertEqual(updated[19], 0)
        self.assertEqual(updated[26:28], bytes.fromhex('00 03'))
        for offset in (2, 7, 14, 15, 16, 17, 18):
