...
Klipper 换热端耗时统计扩展。统计每次换热端的总耗时、各阶段耗时（释放旧热端 / 抓取新热端 / 等待加热），按"本次打印"和"历史累计"两个维度汇总，并通过 `save_variables` 持久化。

## 安装

通过 SSH 在打印机上执行下面命令，会把仓库克隆到Home目录并把 Python 模块软链到 Klipper 的 `extras` 目录：
...
- `TOOLCHANGE_TIMER_BEGIN` / `TOOLCHANGE_TIMER_END`：单次换热端计时起止（END 自动累加并持久化）
...
| 维度 | 内容 | 生命周期 |
|---|---|---|
| `current` | 当前进行中的一次换热端 (各阶段 elapsed) | 单次换热端 |
